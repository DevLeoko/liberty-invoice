...
export async function buildInvoicePdf(
	...
	function ppInvoice() {
		return ppColumn([
			...
			ppText(
				t('invoice.dueText', {
					amount: `${currency.shorthand} ${formatFloat(computeTotalWithTax(invoice))}`,
	...
	function ppItemListTotal() {
		...
		return ppColumn(
			[
				ppRow([ppText(t('invoice.subtotal')), ppText(`${currency.format(withoutTax)}`)], {
					mainAxisAlignment: 'space-between',
					width: { relative: 1 },
					div: {
						padding: 2,
					...
					return ppRow(
						taxRate.rate == 0
							...
							: [
									...
									ppText(`${currency.format(withoutTax * (taxRate.rate / 100))}`),
								],
						{
							mainAxisAlignment: 'space-between',
							width: { relative: 1 },
				...
				ppHr(),
				ppRow(
					[ppText(t('invoice.total')), ppText(`${currency.format(computeTotalWithTax(invoice))}`)],
					{
						mainAxisAlignment: 'space-between',
						width: { relative: 1 },
						text: {
